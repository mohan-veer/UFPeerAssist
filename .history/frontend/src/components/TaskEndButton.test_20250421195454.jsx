...
import '@testing-library/jest-dom'; // Import jest-dom for the missing matchers
...
describe('TaskEndButton Component', () => {
  ...
  it('renders end button correctly', () => {
    ...
    // Check button text - use regular assertions that don't require jest-dom
    expect(screen.getByText('End Task')).toBeTruthy();
    
    // Check help text
    expect(screen.getByText('This will send a notification to the task owner to verify completion.')).toBeTruthy();
  });
  ...
  it('shows success message after clicking end button', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('✅ Task completion initiated')).toBeTruthy();
    });
    
    // Check for OTP message
    expect(screen.getByText('An OTP has been sent to the task owner for verification.')).toBeTruthy();
    
    // End button should not be visible anymore
    expect(screen.queryByText('End Task')).toBeNull();
  ...
  it('shows error message when API returns error', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Failed to end task')).toBeTruthy();
    });
  ...
  it('shows error when user email is not found', async () => {
    ...
    // Check error message
    expect(screen.getByText('You must be logged in to end this task')).toBeTruthy();
  });
  ...
  it('shows error when user is not authorized', async () => {
    ...
    // Check error message
    expect(screen.getByText('You are not authorized to end this task')).toBeTruthy();
  });
