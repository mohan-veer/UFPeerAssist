...
describe('TaskApplications Component', () => {
  ...
  test('shows correct number of people selected', async () => {
    ...
    await waitFor(() => {
      // Looking at the DOM structure in the error, we need to find the parent divs
      const peopleSelectedElements = screen.getAllByText(/People Selected/);
      
      // Get the parent div for each "People Selected" element
      const firstPeopleSelectedDiv = peopleSelectedElements[0].closest('div');
      const secondPeopleSelectedDiv = peopleSelectedElements[1].closest('div');
      
      // Check that each div's complete text contains the expected ratio
      expect(firstPeopleSelectedDiv.textContent).toContain('1');
      expect(firstPeopleSelectedDiv.textContent).toContain('2');
      expect(firstPeopleSelectedDiv.textContent).toContain('/');
      
      expect(secondPeopleSelectedDiv.textContent).toContain('1');
      expect(secondPeopleSelectedDiv.textContent).toContain('1');
      expect(secondPeopleSelectedDiv.textContent).toContain('/');
    });
  });
