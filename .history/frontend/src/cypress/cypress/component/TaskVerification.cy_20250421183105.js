...
describe('TaskVerification Component', () => {
  ...
  it('formats date correctly', () => {
    ...
    cy.contains('div', /Date:/).should(($div) => {
      // Check for various date formats (to account for different locales)
      const text = $div.text();
      const containsFormattedDate = 
        /April\s*20,\s*2025/.test(text) || // US format
        /20\s*April\s*2025/.test(text) ||  // UK format
        /4\/20\/2025/.test(text) ||        // US short format
        /20\/4\/2025/.test(text);          // EU short format
      
      expect(containsFormattedDate).to.be.true;
    });
  });
});